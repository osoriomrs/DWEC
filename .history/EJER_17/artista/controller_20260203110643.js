...
router.post("/save", (req, res) => {
  ...
  if (id) {
    ...
    artista.fecha_formacion = fecha_formacion;
    artista.foto = foto;
  } else {
    artistas.push({
      id: Date.now(),
...
router.get("/:id", (req, res) => {
  const artista = artistas.find(a => a.id == req.params.id);
  const albumesArtista = albumes.filter(al => al.artistaId == artista.id);
  res.send(renderDetalleArtista(artista, albumesArtista));
});
