...
function actualizarRuta() {
  ...
  for (let i = 1; i < puntos.length; i++) {
    distanciaTotal += haversineDistance(puntos[i-1], puntos[i]);
  }
  distanciaTotalElem.textContent = `Distancia total: ${distanciaTotal.toFixed(2)} km`;
  linea.setLatLngs(puntos.map(p => [p.lat, p.lon]));
}

function eliminarPunto(index, li) { //He añadido esta funcion ya que si se introducia una direccion que no se encuentra 
// la linea atravesaba todo el mapa de oeste a este y no se podia quitar, asi se implementa un btn eliminar para quitar el marcador y la linea del mapa
...
async function añadirPunto() {
  ...
  const marker = L.marker([lat, lon]).addTo(mapa).bindPopup(direccion);

  const punto = { lat, lon, direccion, marker };
  puntos.push(punto);
  ...
  btnEliminar.addEventListener("click", () => eliminarPunto(puntos.indexOf(punto), li));
